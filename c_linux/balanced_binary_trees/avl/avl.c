...
int depth2(struct node *tree, int height) {
	printf("\nvalue: %d, height: %d", tree -> value, tree -> height);
	int h1 = 0, h2 = 0;
	if (tree -> left) {
		h1 = depth2(tree -> left, height + 1);
	}
	if (tree -> right) {
		h2 = depth2(tree -> right, height + 1);
	}
